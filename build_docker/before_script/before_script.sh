...
readonly CI_PULL_REGISTRY=${CI_PULL_REGISTRY:-$CI_REGISTRY}
readonly CI_PULL_REGISTRY_USER=${CI_PULL_REGISTRY_USER:-$CI_REGISTRY_USER}
readonly CI_PULL_REGISTRY_PASSWORD=${CI_PULL_REGISTRY_PASSWORD:-$CI_REGISTRY_PASSWORD}
...
main()
{
    ...
    DOCKER_CFG="{'auths':{'$CI_REGISTRY':{'auth':'$(echo -n $CI_REGISTRY_USER:$CI_REGISTRY_PASSWORD|base64 -w 0)'},'$CI_PULL_REGISTRY':{'auth':'$(echo -n $CI_PULL_REGISTRY_USER:$CI_PULL_REGISTRY_PASSWORD|base64 -w 0)'},'https://index.docker.io/v1/':{'auth':'$(echo -n $DOCKERHUB_USER:$DOCKERHUB_TOKEN|base64 -w 0)'}},'proxies':{'default':{'httpProxy':'$http_proxy','httpsProxy':'$https_proxy','noProxy':'$NO_PROXY'}}}"
    ...
    export DOCKER_FILE_DIGEST_NAME="${NAME}_metadata.json"
    export DOCKER_FILE_DIGEST="/tmp/${DOCKER_FILE_DIGEST_NAME}"
    #
    if [ "$REGISTRY_MIRROR" != "" ]
    then
  ...
  mirrors = ['"$REGISTRY_MIRROR"']
    ...
    export DOCKER_BUILD_ARGS="--opt build-arg:parent_version=$PARENT_VERSION --opt build-arg:build_branch=$BUILD_BRANCH --opt build-arg:ci_job_token=$CI_JOB_TOKEN --opt build-arg:registry=$CI_PULL_REGISTRY $OTHER_DOCKER_ARGS"
