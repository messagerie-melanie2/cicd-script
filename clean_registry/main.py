...
def main(args) :
    ...
    #####
    if(args.delete_ghost_image and len(registry)!= 0 and len(dockerfiles_to_build) != 0):
        clean_ghost_images(registry,dockerfiles_to_build,args.token, args.project_id)

    if(args.delete_dev_image and len(registry)!= 0 and len(dockerfiles_to_build) != 0):
